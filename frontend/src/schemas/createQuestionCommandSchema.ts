...
export const createQuestionCommandSchema = z
  .object({
    ...
    options: z.array(createQuestionOptionSchema).min(2),
...
export interface CreateQuestionCommandSchemaOptions {
 ...
 */
export function createCreateQuestionCommandSchema(options: CreateQuestionCommandSchemaOptions = {}) {
  const { certificationExamIds, domainIdsByCertification, topicIdsByCertification } = options

  return createQuestionCommandSchema.superRefine((data, ctx) => {
